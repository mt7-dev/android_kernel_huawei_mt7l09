...
{
	printk("print current tx packet: ");
...
static void respond_cflag80_packet_work(struct work_struct *work)
{
	...
	if (ret) {
		if (-1 == ret) {
			err_flag = 1;
			hwlog_err("%s: respond_cflag80_packet_work has wait data ack timeout.\n", __func__);
		} else if (-2 == ret) {
	...
	do{
		...
		if (ret_read_wait_ack) {
			if (-1 == ret_read_wait_ack) {
				err_flag = 1;
				hwlog_err("%s: spi_read_port_work has wait data ack timeout. xfer.len = %d\n", __func__, xfer.len);
