...
extern MN_CALL_VP_NV_CFG_STATE_ENUM_U8 g_enVpNvCfgState;
...
)
{
    ...
    *penCause = MN_CALL_USER_BUSY;
    ...
    {
        if ((VOS_TRUE == gucTafCallStatusControl)
            ||(VOS_TRUE == gucTafMultiSimCallStatusControl))
        {
            return VOS_TRUE;
        }
    ...
    else if (MN_CALL_TYPE_VIDEO == enCallType)
    {
        ...
        if (( MN_CALL_VP_MO_MT_BOTH == g_enVpNvCfgState )
         || ( MN_CALL_VP_MT_ONLY == g_enVpNvCfgState ))
        {
            return VOS_TRUE;
        }
...
VOS_VOID  TAF_CALL_ProcMnccSrvccStart(VOS_VOID)
{
    ...
    vos_printf("TAF_CALL_ProcMnccSrvccStart, call imsa api time length,tick = %d\r\n", ulSlice);
