 ...
 VOS_VOID TAF_MMA_InitRssiValue(VOS_VOID)
{
    ...
    gstMmaValue.stRssiValue.ucRssiNum               = 0;
    gstMmaValue.stCerssiValue.ucRssiNum             = 0;
    gstMmaValue.stCerssiValue.enRatType             = TAF_PH_RAT_BUTT;
    gstMmaValue.stCerssiValue.ucCurrentUtranMode    = NAS_UTRANCTRL_UTRAN_MODE_BUTT;

    for ( i = 0; i < TAF_PH_RSSI_MAX_NUM; i++ )
    {
        gstMmaValue.stRssiValue.astRssi[i].ucChannalQual    = TAF_MMA_UTRA_BLER_INVALID;
        gstMmaValue.stRssiValue.astRssi[i].lRssiValue       = TAF_MMA_RSSI_LOWER_LIMIT;
        gstMmaValue.stRssiValue.astRssi[i].ucRssiValue4AT   = 0;

        gstMmaValue.stCerssiValue.aRssi[i].ucRssiValue      = 0;
        gstMmaValue.stCerssiValue.aRssi[i].u.stWCellSignInfo.sRscpValue = TAF_MMA_UTRA_RSCP_UNVALID;
        gstMmaValue.stCerssiValue.aRssi[i].u.stWCellSignInfo.sEcioValue = 0;
    }
...
VOS_VOID TAF_MMA_InitOrigMeInfo( VOS_VOID )
{
    ...
    /*获取硬件模块名*/
    lRslt = DRV_MEM_VERCTRL((VOS_INT8 *)(gstMmaValue.stMeInfo.ModelId.aucModelId),
    ...
    /*获取软件版本号*/
    lRslt = DRV_MEM_VERCTRL((VOS_INT8 *)(gstMmaValue.stMeInfo.SoftVersion.aucRevisionId),
                    TAF_MAX_REVISION_ID_LEN + 1,
                    VER_SOFTWARE,
                    VERIONREADMODE);
    if ( DRV_INTERFACE_RSLT_OK != lRslt )
