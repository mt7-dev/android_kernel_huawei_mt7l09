...
VOS_VOID NAS_EMM_IntraEstReq(const NAS_EMM_MRRC_DATA_REQ_STRU *pMsg)
{
    ...
    /* 进入RRC_CONN_EST_INIT.RRC_CONN_WAIT_EST_CNF状态 */
                                        ...
                                        VOS_VOID   *pMsgStru )
{
    ...
    /*系统消息不被禁，根据压栈的状态对系统消息进行不同处理:
      1)ATTACH过程:直接丢弃
      2)Service和DETACH过程，高优先级缓存
      3)TAU过程，周期性TAU且Ta变化则缓存，否则直接丢弃*/
                                                              ...
                                                              VOS_VOID   *pMsgStru  )
{
    ...
    /* (1)如果建链之前是SER_INIT+WAIT_CN_CNF状态,则缓存ESM消息*/
    /* (2)如果建链之前是TAU_INIT+WAIT_CN_CNF状态,且TAU是打断SERVICE流程发起的,则缓存ESM消息*/
