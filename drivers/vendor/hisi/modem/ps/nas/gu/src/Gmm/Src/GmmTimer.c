...
#if 0
/***********************************************************************
  Module   : Gmm_TimerCreat
  Function : GMM使用的定时器创建
  Input    : 无
  Output   : 无
  NOTE     : 无
  Return   : 无
 ...
 ************************************************************************/
VOS_VOID Gmm_TimerCreat (VOS_VOID)
{
    VOS_UINT8        i;                                                             /* 定义循环变量                             */
    VOS_UINT32        aulSndMsg[4];                                                  /* 发送的消息                               */
    VOS_INT32         lResult;                                                       /* 定义变量保存TIMER创建结果                */

    aulSndMsg[0] = WUEPS_MSG_TYPE_TIMER;                                        /* 消息类型为定时器消息                     */
    aulSndMsg[1] = 0;                                                           /* TIMER ID                                 */
    aulSndMsg[2] = 0;                                                           /* 保留                                     */
    aulSndMsg[3] = 0;                                                           /* 保留                                     */
    for (i=0; i<GMM_TIMER_NUM; i++)
    {
        aulSndMsg[1] = i;                                                       /* TIMER ID                                 */
        lResult = WuepsTimerMsgCre(&g_GmmTimerMng.aTimerInf[i].TimerId,
                                   g_GmmTimerMng.aTimerInf[i].ulTimerVal,
                                   VOS_MSG_QUEUE_HANDLER_GMM,
                                   (VOS_INT8 *)aulSndMsg,
                                   16,
                                   WUEPS_MSG_PRIO_A
                                   );                                           /* 创建保护TIMER                            */
        if (WUEPS_TIMER_CREATE_FAILURE == lResult)
        {                                                                       /* 保护TIMER创建失败                        */
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_TimerCreat:WARNING: Timer is fail to be created");
            return;                                                             /* 返回                                     */
        }
    }
}
...
/***********************************************************************
  Module   : Gmm_TimerDistroy
  Function : GMM使用的定时器销毁
  Input    : 无
  Output   : 无
  NOTE     : 无
  Return   : 无
  History  :
     1.  张志勇  2003.12.10  新版作成
 ************************************************************************/
VOS_VOID Gmm_TimerDistroy (VOS_VOID)
{
    VOS_UINT8        i;                                                             /* 定义循环变量                             */
    VOS_INT32         lResult;                                                       /* 定义变量保存TIMER销毁结果                */

    for (i=0; i<GMM_TIMER_NUM; i++)
    {                                                                           /* 循环查找所有定时器                       */
        if (0 != (g_GmmTimerMng.ulTimerRunMask & (0x00000001 << i)))
        {                                                                       /* 该定时器已经启动                         */
            if (WUEPS_TIMER_STOP_FAILURE ==
                WuepsTimerStop(&g_GmmTimerMng.aTimerInf[i].TimerId))
            {                                                                   /* 停止失败：已经停止或Timer不存在          */
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_TimerDistroy:WARNING: Timer is fail to be stoped");
                return;                                                         /* 返回                                     */
            }
            g_GmmTimerMng.ulTimerRunMask &= ~(VOS_UINT32)(0x00000001 << i);                 /* 清Timer的启动标志                        */
        }
        lResult = WuepsTimerDestroy(&g_GmmTimerMng.aTimerInf[i].TimerId);       /* 销毁保护TIMER                            */
        if (WUEPS_TIMER_DESTROY_FAILURE == lResult)
        {                                                                       /* 保护TIMER销毁失败                        */
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_TimerDistroy:WARNING: Timer is fail to be destoryed");
            return;                                                             /* 返回                                     */
        }
    }
}
