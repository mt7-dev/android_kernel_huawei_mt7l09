...
/*******************************************************************************
  Module   : Gmm_SndMsg
  Function : GMM的原语发送
  Input    : MSGQUE_ID  *pMsgQueId  发送消息的目的队列ID
             VOS_VOID       *pMsgAdr    发送消息的首地址
  Output   : 无
  NOTE     : 无
  Return   : 无
  History  :
...
*******************************************************************************/
VOS_VOID Gmm_SndMsg(
                MSGQUE_ID *pMsgQueId,                                           /* 发送消息的目的队列ID的指针               */
                VOS_VOID      *pMsgAdr                                              /* 发送消息的首地址                         */
                )
{
    VOS_UINT32                   aulSndMsg[4];                                       /* 发送消息使用的临时内存定义               */
    VOS_INT32                    lSndRet;                                            /* 原语发送函数返回值                       */

    aulSndMsg[0] = WUEPS_MSG_TYPE_NORMAL;                                       /* MESSAGE TYPE: NORMAL NAS                 */
    aulSndMsg[1] = (VOS_UINT32)pMsgAdr;                                              /* MESSAGE POINTER                          */
    aulSndMsg[2] = 0;                                                           /* RESERVE                                  */
    aulSndMsg[3] = 0;                                                           /* RESERVE                                  */

    lSndRet = WuepsSendMsgNoWait(pMsgQueId,
        (const VOS_INT8 *)aulSndMsg,
        4 * sizeof(VOS_UINT32),
        WUEPS_MSG_PRIO_A);                                                      /* 发送消息                                 */
...
VOS_VOID Gmm_SndMmcServiceStatusInd()
{
    MMCGMM_SERVICE_STATUS_IND_STRU             *pstMsg          = VOS_NULL_PTR;
    MMCGMM_SERVICE_STATUS_IND_STRU             *pstInternalMsg  = VOS_NULL_PTR;
    VOS_UINT32                          ulLen;

    /* 从内部消息队列中获取一个还没有使用的空间 */
    ulLen           = sizeof(MMCGMM_SERVICE_STATUS_IND_STRU);

    pstInternalMsg  = (MMCGMM_SERVICE_STATUS_IND_STRU*)NAS_MML_GetIntMsgSendBuf(ulLen);
    if ( VOS_NULL_PTR == pstInternalMsg)
    {
        NAS_ERROR_LOG(WUEPS_PID_GMM, "Gmm_SndMmcServiceStatusInd,ERROR:ALLOC BUFFER WRONG!");
        return;
    }
    ...
    pstMsg->MsgHeader.ulReceiverPid       = WUEPS_PID_MMC;
    pstMsg->MsgHeader.ulLength            = ulLen - VOS_MSG_HEAD_LENGTH;
    pstMsg->MsgHeader.ulMsgName           = MMCGMM_SERVICE_STATUS_IND;
    pstMsg->ulServiceStatus               = Gmm_ComServiceSts();
                       ...
                       )
{
    ...
    {                                                                           /* 需要判断类型的                           */
        ...
        {                                                                       /* UeId存在标识中P_TMSI有效                 */
            ...
            else if (GMM_FALSE == Gmm_Com_CmpRai(&stRai, g_GmmGlobalCtrl.ucNetMod))
            {                                                                   /* RAI不同                                  */
