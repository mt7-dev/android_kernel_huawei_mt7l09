...
VOS_VOID NAS_MM_BackupCcoFallbackCtx(VOS_VOID)
{
    ...
    g_MmGlobalInfo.stBackupShare.ulT3212Value               = g_MmGlobalInfo.ulT3212Value;
...
VOS_VOID NAS_MM_ResumeCcoFallbackCtx(VOS_VOID)
{
    ...
    g_MmGlobalInfo.ulT3212Value     = g_MmGlobalInfo.stBackupShare.ulT3212Value;
...
)
{
    ...
    if (MMC_SUSPEND_CAUSE_HANDOVER == g_MmGlobalInfo.stSuspendShare.ucSuspendCause)
    {
        ...
        else
        {
            ...
            {
                /* 启动等系统消息的定时器 */
                Mm_TimerStart(MM_TIMER_EMERGENCY_CSFB_HO_WAIT_SYSINFO);
            }
            else
            {
                Mm_TimerStart(MM_TIMER_HO_WAIT_SYSINFO);
            }
        }
    }
