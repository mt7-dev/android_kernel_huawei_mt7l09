...
{
    /* 发送消息内容变量 */
    LL_NAS_UNITDATA_REQ_MSG     *pDataReqMsg;               /* 待发送的消息指针 */
    VOS_UINT32                   ulRet;
    ...
    {/* 无效指针 */
        /* 打印错误信息 */
        GMM_LOG_ERR("GMM_SndLlcDataReq():Error: Invaild PDU pointer");
        return;                         /* 返回 */
    }

    /* 创建发送消息 */
    pDataReqMsg = (LL_NAS_UNITDATA_REQ_MSG *)PS_ALLOC_MSG(WUEPS_PID_GMM,
                    ((sizeof(LL_NAS_UNITDATA_REQ_MSG) - VOS_MSG_HEAD_LENGTH)
                    - 1) + usPduLen);
    ...
    pDataReqMsg->ulReceiverPid   = UEPS_PID_LL;

    /* 填写消息发送参数 */
    pDataReqMsg->usMsgType = ID_LL_UNITDATA_REQ;            /* LL_UNITDATA_REQ 消息ID */
    pDataReqMsg->usTransId = GMM_LLC_TRANSID;               /* 子进程，这里填SAPI, GMM使用1 */
    pDataReqMsg->stUnitDataReq.ulTlli = gstGmmCasGlobalCtrl.ulTLLI;   /* ulTLLI参数 */

    /* 用填写消息内容 */
    pDataReqMsg->stUnitDataReq.ucRadioPri = ucRadioPri;     /* ucRadioPri参数 */
    pDataReqMsg->stUnitDataReq.ucCiphInd  = ucCiphInd;      /* ucCiphInd参数 */
    pDataReqMsg->stUnitDataReq.usPduLen   = usPduLen;       /* usPduLen参数 */
    Gmm_MemCpy(pDataReqMsg->stUnitDataReq.aucPdu,
               pucPdu, usPduLen);

    ulRet = PS_SEND_MSG(WUEPS_PID_GMM, pDataReqMsg);
    if (VOS_OK != ulRet)
    {
        /* 打印错误信息 */
        GMM_LOG_ERR("GMM_SndLlcDataReq():Error: Failed in VOS_SendMsg()");
    }
...
VOS_VOID GMM_SndRrmmDataReqGsm(NAS_MSG_STRU *pMsg)
{
    ...
    if ( (GMM_DEREGISTERED_INITIATED == g_GmmGlobalCtrl.ucState)
       /*||(GMM_ROUTING_AREA_UPDATING_INITIATED == g_GmmGlobalCtrl.ucState)*/
        ||(0x0 != (g_GmmGlobalCtrl.ucState & 0xF0)) )
       /*||(0x20 == (g_GmmGlobalCtrl.ucState & 0xF0)) )*/
    /*modify,sunxibo,2006-02-23,itt begin*/
    {
        if (0 == gstGmmCasGlobalCtrl.ucGprsCipherAlg)
    ...
    {
        if (0x08 == pMsg->aucNasMsg[0])
        {
            switch(pMsg->aucNasMsg[1])
            {
                case 0x01:
                case 0x04:
                case 0x12:
