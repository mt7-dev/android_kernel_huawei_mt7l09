...
VOS_UINT32 NAS_SMS_ResendPsRpData(VOS_UINT8 ucRetransFlg)
{
    ...
    if (NAS_GMM_NET_RAT_TYPE_LTE == GMM_GetCurNetwork())
    {
        ...
        /* 发送CP-DATA消息 */
        NAS_SMS_SndLmmCpDataReq(g_SmcPsEnt.SmcMo.HoldSndMsg.pMsg, g_SmcPsEnt.SmcMo.HoldSndMsg.ulMsgLen);
    }
    else
#endif
    {
        ...
        SMC_SndGmmMsg(g_SmcPsEnt.SmcMo.HoldSndMsg.pMsg,
                      PMMSMS_UNITDATA_REQ,
                      (VOS_UINT16)g_SmcPsEnt.SmcMo.HoldSndMsg.ulMsgLen) ;             /* 重发消息                                 */
    }
...
VOS_UINT32 NAS_SMS_ResendPsRpReport(VOS_VOID)
{
    ...
    if (NAS_GMM_NET_RAT_TYPE_LTE == GMM_GetCurNetwork())
    {
        NAS_SMS_SndLmmCpDataReq(g_SmcPsEnt.SmcMt.HoldSndMsg.pMsg, g_SmcPsEnt.SmcMt.HoldSndMsg.ulMsgLen);
    }
    else
#endif
    {
        SMC_SndGmmMsg(g_SmcPsEnt.SmcMt.HoldSndMsg.pMsg,
                      PMMSMS_UNITDATA_REQ,
                      (VOS_UINT16)g_SmcPsEnt.SmcMt.HoldSndMsg.ulMsgLen) ;             /* 重发消息                                 */
    }
