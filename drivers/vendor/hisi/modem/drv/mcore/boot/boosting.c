...
s32 boosting(void)
{
	...
	struct hpm_tem_print *hpm_temp_print_ptr = ((struct hpm_tem_print * )DUMP_EXT_BOOST_MCORE_ADDR);
...
void dfs_to_max(void)
{
	...
	writel(0x64514051, CM3_DDRC_BASE_ADDR + 0x78);
	...
	/* 9 */
	writel(0x00600000, CM3_CRG_BASE_ADDR + HI_CRG_DFS1_CTRL1_OFFSET);
	writel(0x02101045, CM3_CRG_BASE_ADDR + HI_CRG_DFS1_CTRL2_OFFSET);
    times = 30;
    while(times)
    {
        times--;
