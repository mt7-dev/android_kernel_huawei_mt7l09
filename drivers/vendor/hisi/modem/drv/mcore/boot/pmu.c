...
s32 bsp_pmu_volt_set(s32 volt_id,s32 voltage_mv)
{
    ...
    switch(volt_id)
    {
        case PMIC_HI6559_BUCK3:
            if(925 == voltage_mv)
            {
                bsp_hi6559_reg_write(HI6559_VSET_BUCK3_ADJ_OFFSET,0x1D);/* 932mv */
            }
            else if(950 == voltage_mv)
            {
                bsp_hi6559_reg_write(HI6559_VSET_BUCK3_ADJ_OFFSET,0x20);/* 956mv */
            }
            else
            {
                iret = -1;
        ...
        case PMIC_HI6559_LDO03:
            if(950 == voltage_mv)
            {
                bsp_hi6559_reg_write(HI6559_LDO3_REG_ADJ_OFFSET,0x05);/* 950mv */
            }
            else
            {
                iret = -1;
