...
s32 bsp_hi6559_dr_set_mode(s32 dr_id, dr_mode_e mode)
{
    ...
    case PMU_DRS_MODE_BRE_FLASH:
        ...
        else
        {
            ...
            regval |= (u8)(0x1 << hi6559_dr->bre_mode_bit_offset);
...
dr_mode_e bsp_hi6559_dr_get_mode(s32 dr_id)
{
    ...
    /*先判断是闪烁模式还是呼吸模式*/
    bsp_dr_reg_read(hi6559_dr->mode_sel_reg_addr, &regval);
    if(regval & ((u8) (0x1 << hi6559_dr->mode_sel_bit_offset)))
    {
        /*判断是闪烁长亮还是闪烁闪亮*/
        bsp_dr_reg_read(hi6559_dr->fla_mode_reg_addr, &regval);
        if(regval & ((u8) (0x1 << hi6559_dr->fla_mode_bit_offset)))
        {
    ...
    else
    {
        ...
        else
        {
            /*判断是呼吸长亮还是呼吸闪烁*/
            bsp_dr_reg_read(hi6559_dr->bre_mode_reg_addr, &regval);
            if(regval & ((u8) (0x1 << hi6559_dr->bre_mode_bit_offset)))
            {
...
s32 bsp_hi6559_dr_is_enabled(s32 dr_id)
{
    ...
    if(PMU_DRS_MODE_FLA_FLASH == bsp_hi6559_dr_get_mode(dr_id) || PMU_DRS_MODE_FLA_LIGHT == bsp_hi6559_dr_get_mode(dr_id))/*FLA MODE*/
    {
        ...
        if (PMU_DR03 <= dr_id)/*dr3/4/5闪烁模式下开启还需要判断bre开关寄存器*/
        {
            ...
            return ((regval2 & (0x1<<hi6559_dr->bre_onoff_bit_offset))&&(regval & (0x1<<hi6559_dr->fla_onoff_bit_offset)));
        }
        else
            return (regval & (0x1<<hi6559_dr->fla_onoff_bit_offset));
    }
    else/*BRE MODE*/
    {
        bsp_dr_reg_read(hi6559_dr->bre_onoff_reg_addr, &regval);
        return (regval & (0x1<<hi6559_dr->bre_onoff_bit_offset));
    }
...
s32 hi6559_dr_start_delay_set(dr_id_e dr_id, u32 delay_ms)
{
    ...
    hi6559_dr = &hi6559_dr_attr[dr_id];
    bsp_dr_reg_write(hi6559_dr->start_del_reg_addr,delay_ms/DR345_START_DELAY_STEP);
