...
static void MD5_memset (uint8 * output, int16 value, uint32 len);
...
static void MD5Transform (uint32 state[4], uint8 block[64])
{
  uint32 a = state[0], b = state[1], c = state[2], d = state[3], x[16]={0};
...
/*========================================================================
FUNCTION       encrypt_simlock_md5_data

DESCRIPTION  : 采用hash_md5算法计算MD5数据
                 ...
                 void* output_data)
{
  ...
  hash_md5_update(&context, (uint8*)input_data, input_length);
  hash_md5_final(&context, (uint8*)output_data);
}
