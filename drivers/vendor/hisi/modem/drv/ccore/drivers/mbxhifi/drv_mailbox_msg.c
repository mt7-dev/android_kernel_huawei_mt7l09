                ...
                unsigned long            length)
{
   ...
   if ((0 == pdata) || (0 == length)) {
    ...
    /*封信*/
    ret_val = (unsigned long)mailbox_sealup_buff( mb_buf,  length);
    if (MAILBOX_OK == ret_val) {
         /*发送邮件*/
        ret_val = (unsigned long)mailbox_send_buff(mb_buf);
    }
                ...
                unsigned long          *size)
{
    ...
    *size =  (unsigned long)mailbox_read_buff(pMailQueue, buff, pMailQueue->size);
                ...
                unsigned long            length)
{
    ...
    if (MAILBOX_FALSE == mailbox_int_context()) {
        ...
        while (MAILBOX_FULL == ret_val) {
            (void)mailbox_delivery(mailbox_get_channel_id(mailcode));
            try_go_on = (unsigned long)mailbox_scene_delay(MAILBOX_DELAY_SCENE_MSG_FULL, &try_times);
