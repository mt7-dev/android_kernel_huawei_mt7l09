    ...
    VOS_CHAR                           *pucDest)
{
    ...
    *(VOS_UINT32 *)stEncrypt.aucEncryptContent              =   ulItemLen;
    ...
    lResult = CRYPTO_HASH((VOS_CHAR*)(stEncrypt.aucEncryptContent),
    ...
    {
        SC_ERROR2_LOG("SC_APSEC_ItemEncrypt: CRYPTO_HASH is Failed.", lResult, lHashLen);
    ...
    lResult = CRYPTO_ENCRYPT((VOS_CHAR*)(stEncrypt.aucEncryptContent),
    ...
    if ((sizeof(SC_APSEC_ENCRYPT_STRU) != lEncrypeCodeLen)
        || (VOS_OK != lResult))
    {
        SC_ERROR2_LOG("SC_APSEC_ItemEncrypt: CRYPTO_ENCRYPT is failed.", lResult, lEncrypeCodeLen);
    ...
    SC_APSEC_DEFAULT_ITEM_STRU         *pstSecItem)
{
    ...
    /* 为整个文件分配内存，后续一次写入 */
    pucFile         =   (VOS_CHAR *)VOS_MemAlloc(WUEPS_PID_OM,
                                                 DYNAMIC_MEM_PT,
    ...
    for (i = 0; i < SC_SECURITY_ITEM_NUM; i++)
    {
                                            ...
                                            pucFile + (i * sizeof(SC_APSEC_ENCRYPT_STRU))))
        {
            ...
            VOS_MemFree(WUEPS_PID_OM, pucFile);
    ...
    {
        VOS_MemFree(WUEPS_PID_OM, pstSecItemAddr->pucContent);
    }

    pstSecItemAddr->pucContent      =   (VOS_CHAR *)VOS_MemAlloc(WUEPS_PID_OM,
                                                                 STATIC_MEM_PT,
    ...
    lResult = CRYPTO_HASH((VOS_CHAR*)pstSecItem,
                          (VOS_INT)(SC_SECURITY_ITEM_CONTENT_SIZE + SC_SECURITY_ITEM_LENGTH_SIZE),
                          CRYPTO_ALGORITHM_SHA256,
    ...
    if ((VOS_OK != lResult)
        || (SC_HASH_LEN != lHashLen))
    {
        SC_ERROR2_LOG("SC_APSEC_UpdateSecItem: CRYPTO_HASH is Failed.", lResult, lHashLen);
...

{
    ...
    pstEncryptFile  =   (SC_APSEC_ENCRYPT_STRU *)VOS_MemAlloc(WUEPS_PID_OM,
                                                              DYNAMIC_MEM_PT,
                                                              SC_SECURITY_ITEM_NUM * sizeof(SC_APSEC_ENCRYPT_STRU));
                                   ...
                                   SC_SECURITY_ITEM_NUM * sizeof(SC_APSEC_ENCRYPT_STRU)))
    {
        ...
        {
            VOS_MemFree(WUEPS_PID_OM, pstEncryptFile);
    ...
    SC_APSEC_DEFAULT_ITEM_STRU         *pstSecItem)
{
    ...
    /* 用ASSK和AES-ECB算法解密 */
    lResult         =   CRYPTO_DECRYPT((VOS_CHAR*)(pstEncryptItem),
                                       sizeof(SC_APSEC_ENCRYPT_STRU),
                                       CRYPTO_ENCRYPT_ALGORITHM_AES_ECB,
                                       (VOS_CHAR *)aucKey,
                                       (VOS_INT)ulKeyLen,
    ...
    if ((lDecryptLen != sizeof(SC_APSEC_ENCRYPT_STRU))
      || (VOS_OK != lResult))
    {
        SC_ERROR2_LOG("SC_APSEC_ReadEncryptItem: CRYPTO_DECRYPT failed, lResult %X, lDecryptLen %X", lResult, lDecryptLen);
    ...
    lResult     =   CRYPTO_HASH((VOS_CHAR *)(&stDecryptItem),
    ...
    if ((VOS_OK != lResult)
      ||(SC_HASH_LEN != lHashLen))
    {
        SC_ERROR2_LOG("SC_APSEC_ReadEncryptItem: CRYPTO_DECRYPT failed, lResult %X, lHashLen %X", lResult, lHashLen);
...
VOS_UINT32  SC_APSEC_InitDualBackFile(VOS_VOID)
{
    ...
    if (VOS_NULL_PTR == pstEncryptFileC)
    {
        ...
        VOS_MemFree(WUEPS_PID_OM, pstEncryptFileA);
    ...
    for (i = 0; i < SC_SECURITY_ITEM_NUM; i++)
    {
        ...
        {
            VOS_MemFree(WUEPS_PID_OM, pstApSecItemAddr->pucContent);
        ...
        pstApSecItemAddr->pucContent    =   (VOS_CHAR *)VOS_MemAlloc(WUEPS_PID_OM, STATIC_MEM_PT, pstApSecItemAddr->ulItemLen);
    ...
    /* 释放之前申请的所有内存 */
    VOS_MemFree(WUEPS_PID_OM, pstEncryptFileA);
    VOS_MemFree(WUEPS_PID_OM, pstEncryptFileC);
    ...
    SC_APSEC_DEFAULT_ITEM_STRU         *pstSecItem)
{
    ...
    if ((VOS_OK != lResult)
      ||(SC_HASH_LEN != lHashLen))
    {
        SC_ERROR2_LOG("SC_APSEC_ReadSecItem: CRYPTO_HASH fail lResult %d, lHashLen.", lResult, lHashLen);
...
VOS_UINT32  SC_APSEC_FreshDualBackFile(VOS_VOID)
{
    ...
    pucFile =   (VOS_UINT8 *)VOS_MemAlloc(WUEPS_PID_OM,
                                          DYNAMIC_MEM_PT,
                                          SC_SECURITY_ITEM_NUM * sizeof(SC_APSEC_ENCRYPT_STRU));
    ...
    VOS_UINT8                          *pucSecPacket )
{
    ...
    lLen            = SC_KEY_LEN;
    lRetrun         = CRYPTO_ENCRYPT(acSecureCode,
                                     sizeof(acSecureCode),
                                     CRYPTO_ENCRYPT_ALGORITHM_AES_ECB,
                                     (VOS_CHAR*)aucSATK,
                                     SC_KEY_LEN,
    ...
    lRetrun         = CRYPTO_ENCRYPT((VOS_CHAR*)pstSecPacket->aucData,
    ...
    SC_APSEC_SECURE_PACKET_STRU        *pstSecPacket )
{
    ...
    lResult         = CRYPTO_DECRYPT((VOS_CHAR*)pucCipherSP,
    ...
    VOS_UINT32                         *pulCipherSPLen)
{
    ...
    /* 调用DRV的加密接口 */
    lResult         = CRYPTO_ENCRYPT((VOS_CHAR*)pstSecPacket,
                                     (VOS_INT)ulLen,
                                     CRYPTO_ENCRYPT_ALGORITHM_AES_ECB,
                                     (VOS_CHAR*)pucSecATkey,
                                     SC_SEC_AT_KEY_LEN,
